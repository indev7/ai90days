...
import { getDatabase, get, all, run } from './pgdb';
...
export async function propagateTaskProgress(taskId) {
  ...
  try {
    ...
    const task = await get('SELECT parent_id FROM okrt WHERE id = ? AND type = "T"', [taskId]);
    ...
    const kr = await get('SELECT parent_id FROM okrt WHERE id = ? AND type = "K"', [krId]);
