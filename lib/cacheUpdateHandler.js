...
export function processCacheUpdate(response) {
  ...
  if (_cacheUpdate && _cacheUpdate.action) {
    ...
    switch (_cacheUpdate.action) {
      ...
      case 'removeMyOKRTs':
        if (Array.isArray(_cacheUpdate.data?.ids) && _cacheUpdate.data.ids.length > 0) {
          store.removeMyOKRTs(_cacheUpdate.data.ids);
          console.log('Cache updated: Removed OKRTs', _cacheUpdate.data.ids.length);
        }
        break;
