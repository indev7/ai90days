// Database Support: PostgreSQL via Prisma
...
export async function getDatabase() {
  if (DATABASE_PROVIDER !== 'postgres') {
    throw new Error('SQLite is no longer supported. Set DATABASE_PROVIDER=postgres.');
  }
...
export async function getSharedOKRTsForUser(userId) {
  if (isPrisma()) {
    ...
    const parentById = new Map(allGroups.map(group => [group.id, group.parentGroupId]));
    const childrenById = new Map();

    allGroups.forEach(group => {
      if (group.parentGroupId === null || group.parentGroupId === undefined) return;
      if (!childrenById.has(group.parentGroupId)) {
        childrenById.set(group.parentGroupId, []);
      }
      childrenById.get(group.parentGroupId).push(group.id);
    });
    ...
    groupIds.forEach((groupId) => {
      addAncestors(groupId);
      addDescendants(groupId);
    });
    const relatedGroupIdsArray = Array.from(relatedGroupIds);
    ...
    const shares = await prisma.share.findMany({
      where: {
        OR: [
          ...
          { shareType: 'G', groupOrUserId: { in: relatedGroupIdsArray } }
        ]
      },
      include: {
        okrt: {
