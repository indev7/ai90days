...
// Ensure sanitize layer is loaded early on server modules
import './sanitizeLayer.js';
import { sanitizeObject } from './sanitizeLayer.js';
...
export async function getDatabase() {
  if (DATABASE_PROVIDER === 'postgres') {
    ...
    // Wrap db.get and db.all to sanitize returned rows/arrays
    try {
      ...
      // Non-fatal: if binding fails, continue without wrapping
      console.warn('Warning: failed to wrap sqlite db methods for sanitization', e);
    }
...
async function initializeSQLiteSchema(db) {
  try {
    ...
    // Check if we need to fix visibility constraint (should only allow 'private' and 'shared')
    try {
      // Try to insert a test record with 'shared' visibility
      await db.run(`
        INSERT INTO okrt (id, type, owner_id, visibility)
        VALUES ('__test_visibility__', 'O', 1, 'shared')
      `);
      // If successful, delete the test record
      await db.run(`DELETE FROM okrt WHERE id = '__test_visibility__'`);
    } catch (constraintError) {
      if (constraintError.message && constraintError.message.includes('CHECK constraint failed')) {
        // Visibility constraint needs fixing
        console.log('Fixing visibility constraint to allow only private and shared...');
        const migrationPath = join(process.cwd(), 'Phase1', 'DB', 'migrate-visibility-fix.sql');
        const migration = readFileSync(migrationPath, 'utf8');
        await db.exec(migration);
        console.log('Visibility constraint fix completed!');
      }
    }
  } catch (error) {
...
function convertPrismaToSQLite(result) {
  ...
  try {
    return sanitizeObject(converted);
  } catch (e) {
    return converted;
  }
}
...
export async function createUser({ email, password_hash, display_name }) {
  ...
  try { ({ email, password_hash, display_name } = sanitizeObject({ email, password_hash, display_name }) || {}); } catch (e) {}
  if (isPrisma()) {
    const prisma = await getDatabase();
...
export async function updateUser(userId, updateData) {
  try { updateData = sanitizeObject(updateData) || updateData; } catch (e) {}
  if (isPrisma()) {
    const prisma = await getDatabase();
...
export async function createOKRT(okrtData) {
  try { okrtData = sanitizeObject(okrtData) || okrtData; } catch (e) {}
  if (isPrisma()) {
    const prisma = await getDatabase();
    const {
...
export async function createGroup(groupData) {
  try { groupData = sanitizeObject(groupData) || groupData; } catch (e) {}
  if (isPrisma()) {
    const prisma = await getDatabase();
...
export async function createNotification(notificationData) {
  try { notificationData = sanitizeObject(notificationData) || notificationData; } catch (e) {}
  if (isPrisma()) {
    const prisma = await getDatabase();
    const {
...
export async function createComment(commentData) {
  try { commentData = sanitizeObject(commentData) || commentData; } catch (e) {}
  if (isPrisma()) {
    const prisma = await getDatabase();
    const {
...
export async function createTimeBlock(timeBlockData) {
  try { timeBlockData = sanitizeObject(timeBlockData) || timeBlockData; } catch (e) {}
  if (isPrisma()) {
    const prisma = await getDatabase();
    const {
