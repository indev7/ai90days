import { getDbAdapter, getDatabaseType, DB_TYPES } from './db-config.js';
...
export async function getDatabase() {
  ...
  try {
    ...
    if (dbType === DB_TYPES.SQLITE) {
      // Use existing SQLite schema and migrations
    ...
    // Only run migrations for SQLite 
    if (dbType === DB_TYPES.SQLITE) {
...
export async function createUser({ email, password_hash, display_name }) {
  ...
  // Some DB adapters (Postgres) don't expose result.lastID the same way SQLite does.
  // To be DB-agnostic, query the user by their unique email which was just inserted.
  // This guarantees we return a user object for downstream callers (e.g. createSession).
  return database.get('SELECT * FROM users WHERE email = ?', [email]);
}
...
export async function getOKRTHierarchy(ownerId) {
  ...
  // Cast both owner_id and parameter to string for robust comparison across DB types
...
export async function addUserToGroup(userId, groupId, isAdmin = false) {
  ...
  if (database.type === 'postgres') {
    // PostgreSQL upsert using ON CONFLICT on the composite primary key
    return database.run(`
...
export async function shareOKRTWithGroup(okrtId, groupId) {
  const database = await getDatabase();
  if (database.type === 'postgres') {
    // Use ON CONFLICT DO NOTHING to avoid duplicate share entries
    return database.run(`
...
export async function getSharedOKRTsForUser(userId) {
  ...
  return database.all(`
    SELECT DISTINCT o.*, u.display_name as owner_name,
           ...
           CASE WHEN f.id IS NOT NULL THEN 0 ELSE 1 END as follow_sort_order,
           o.updated_at as sort_updated_at
    FROM okrt o
...
export async function createTimeBlock(timeBlockData) {
  const database = await getDatabase();
  const {
    task_id, user_id, start_time, duration
  } = timeBlockData;

  const result = await database.run(
    'INSERT INTO time_blocks (task_id, user_id, start_time, duration) VALUES (?, ?, ?, ?)',
    [task_id, user_id, start_time, duration]
  );
...
export async function getTimeBlocksByUserAndDate(userId, date) {
  ...
  return database.all(`
    SELECT tb.*, 
           ...
           kr.parent_id as objective_id,
           obj.title as objective_title
    FROM time_blocks tb
    JOIN okrt task ON tb.task_id = task.id
    LEFT JOIN okrt kr ON task.parent_id = kr.id
    LEFT JOIN okrt obj ON kr.parent_id = obj.id
