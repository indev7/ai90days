export const MICROSOFT_MAIL_DOMAIN = `# Microsoft Outlook Mail (Graph) Domain
...
## Scope and Consent
- Required delegated scopes: \`Mail.ReadBasic\` + \`Mail.Read\` (message body access).
- If Graph returns 401/403, prompt user to re-login and grant consent.
- Error messages:
  - 401: "Microsoft consent required for Mail.Read; please re-login."
...
### 3) Get Message Body (full read)
\`GET /api/ms/mail/message/:id/body\`
Response includes \`bodyContentType\` and \`body\`.

### 4) Open in Outlook (redirect proxy)
...
- Fetch full \`body\` only when user intent explicitly asks to read message content.
