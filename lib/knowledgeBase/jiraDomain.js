...
## CRITICAL: When to Use LIST_JIRA_TICKETS vs jiraTickets Data

**Use LIST_JIRA_TICKETS action (fetch fresh data from JIRA API):**
- User asks to "show", "list", "find", "get", "fetch" JIRA tickets
- User wants filtered results (by status, project, etc.)
- User wants real-time/current data from JIRA
- User asks about "pending", "open", "in progress" tickets
- **ACTION:** Emit LIST_JIRA_TICKETS action with filters in payload

**Use jiraTickets data section (cached data from mainTree):**
- NEVER request jiraTickets via req_more_info for listing - it may be stale
- Only use if jiraTickets data is ALREADY in context and you need to reference specific ticket details
- Prefer LIST_JIRA_TICKETS action for fresh data

**Example (Correct):**
User: "Show me my pending JIRA tickets"
You: "I'll fetch your tickets!" + emit_jira_actions({actions: [{intent: "LIST_JIRA_TICKETS", endpoint: "/api/jira/tickets", method: "GET", payload: {status: "Open"}}]})

❌ **WRONG:** Calling req_more_info to request jiraTickets data section
✅ **RIGHT:** Emit LIST_JIRA_TICKETS action to fetch fresh tickets

---

## CRITICAL: Output Contract

**TWO-TURN WORKFLOW (MANDATORY):**

**Turn 1 - Tool Request (if emit_jira_actions not available):**
- Call req_more_info to request jira-domain knowledge and emit_jira_actions tool
- Do NOT request jiraTickets data section - it's stale/cached
- Do NOT emit actions yet
- Example: req_more_info({domainKnowledge: {ids: ["jira-domain"]}, tools: {ids: ["emit_jira_actions"]}})

**Turn 2 - Action Emission (REQUIRED after Turn 1):**
- Stream brief coaching text (1-2 sentences)
- **IMMEDIATELY** call emit_jira_actions with actions array
- For listing tickets: Use LIST_JIRA_TICKETS action, NOT req_more_info for jiraTickets data
- Do NOT skip this turn or just acknowledge
- Example: "I'll fetch your tickets!" + emit_jira_actions({actions: [{intent: "LIST_JIRA_TICKETS", endpoint: "/api/jira/tickets", method: "GET", payload: {status: "Open"}}]})

**IF emit_jira_actions is ALREADY AVAILABLE (you see it in your tools list):**
- Skip Turn 1 entirely
- Go directly to Turn 2 (stream text + call tool)

**FORBIDDEN PATTERNS:**
- ❌ "I'll fetch your tickets!" (text only, no tool call)
- ❌ Requesting jiraTickets data section via req_more_info for listing
- ❌ Requesting tools again after they're already available
- ✅ Use LIST_JIRA_TICKETS action to fetch tickets from JIRA API

**Required for EVERY action:**
- \`intent\`: CREATE_JIRA, UPDATE_JIRA, JIRA_ASSIGN_USER, COMMENT_JIRA, TRANSITION_JIRA, CREATE_SUBTASK, LINK_JIRA, CREATE_LEAVE, LIST_JIRA_TICKETS, BULK_TRANSITION_JIRA
- \`endpoint\`: API endpoint (e.g., "/api/jira/tickets")
- \`method\`: HTTP method (POST, PUT, PATCH, GET)
- \`payload\`: Request body object

**Example (correct Turn 2):**
User: "Create a task in D90 called Test Task"
You: Stream "I'll create that task!" AND call emit_jira_actions({actions: [{intent: "CREATE_JIRA", endpoint: "/api/jira/tickets", method: "POST", payload: {project: "D90", summary: "Test Task", issueType: "Task"}}]})
Result: User sees "Accept" button to approve action

---

## CRITICAL: CREATE + ASSIGN WORKFLOW

**When user wants to CREATE a new ticket AND assign it in the same request:**

⚠️ **LIMITATION:** You cannot assign a ticket that doesn't exist yet (ticket key is unknown until after creation).

**SOLUTION - Choose ONE approach:**

**Option 1 (RECOMMENDED):** Create WITHOUT assignment, inform user they can assign after creation
- Emit ONE action: CREATE_JIRA (without assignee in payload)
- In your text response, say: "I've created the ticket. Once it's created, you can ask me to assign it!"

**Option 2:** Create THEN assign using separate user messages
- First message: Create ticket (get ticket key from response)
- Wait for user to ask to assign
- Second message: Assign to user

❌ **FORBIDDEN:** Do NOT emit both CREATE_JIRA and JIRA_ASSIGN_USER in the same actions array if creating a new ticket!
- Reason: Assignment requires the ticket key, which doesn't exist until AFTER creation completes

✅ **EXAMPLE (Correct):**
User: "Create a task in D90 called Test and assign to john@example.com"
You: "I'll create that task! After it's created, you can ask me to assign it to John." + emit_jira_actions({actions: [{intent: "CREATE_JIRA", ...}]})

---

## CRITICAL: ASSIGNMENT RULE (EXISTING TICKETS)

When user asks to assign an **EXISTING** JIRA ticket to any person:
...
✅ CORRECT PATTERN (for existing tickets):
...
#### 7. LIST_JIRA_TICKETS
- **Endpoint:** \`/api/jira/tickets\` (FIXED - do not add query params here)
- **Method:** GET
- **Payload:** Put filter parameters in payload (they will be converted to query params automatically)
  - \`status\`: Filter by status (optional)
  - \`project\`: Filter by project key (optional) - e.g., "D90", "ILT"
  - \`assignee\`: Filter by assignee (optional) - use "currentUser()" for current user
...
- "90Days", "90 Days", "90days", "D90", "ninety days" → project: "D90"
- "Intervest Leave tracker", "Leave tracker", "leave tickets", "leave", "ILT" → project: "ILT"

**Examples:**
\`\`\`json
// Show all open tickets assigned to current user
{
...
// Show done tickets from D90 project
{
  "intent": "LIST_JIRA_TICKETS",
  "endpoint": "/api/jira/tickets",
  "method": "GET",
...
// Show all tickets from 90 Days project (including any status)
{
  "intent": "LIST_JIRA_TICKETS",
  "endpoint": "/api/jira/tickets",
  "method": "GET",
...
// Show all tickets assigned to me (no status or project filter)
{
  "intent": "LIST_JIRA_TICKETS",
  "endpoint": "/api/jira/tickets",
  "method": "GET",
