...
async function get(sql, params = []) {
  ...
  return sanitizeObject(result.rows[0] || null);
}
...
async function all(sql, params = []) {
  ...
  const convertedSql = convertParams(sql);
  const result = await db.query(convertedSql, params);
  return sanitizeObject(result.rows);
}
...
export async function createUser({ email, password_hash, display_name }) {
  ...
  const result = await get(
    'INSERT INTO users (username, password_hash, display_name, email) VALUES (?, ?, ?, ?) RETURNING *',
...
export async function createGroup(groupData) {
  ...
  let { id, name, type, parent_group_id, thumbnail_url } = groupData;
