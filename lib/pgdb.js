...
export async function createOKRT(okrtData) {
  let {
    ...
    status = 'A', area, cycle_qtr, order_index = 0, visibility = 'private',
...
// Jira link helpers (Phase 22)
export async function addJiraLink(okrtId, jiraTicketId) {
  return run(
    'INSERT INTO jira_link (okrt_id, jira_ticket_id) VALUES (?, ?) ON CONFLICT (okrt_id, jira_ticket_id) DO NOTHING',
    [okrtId, jiraTicketId]
  );
}

export async function removeJiraLink(okrtId, jiraTicketId) {
  return run(
    'DELETE FROM jira_link WHERE okrt_id = ? AND jira_ticket_id = ?',
    [okrtId, jiraTicketId]
  );
}

export async function getJiraLinksByOkrtId(okrtId) {
  return all(
    'SELECT okrt_id, jira_ticket_id, created_at FROM jira_link WHERE okrt_id = ? ORDER BY created_at ASC',
    [okrtId]
  );
}

export async function getJiraLinksByOkrtIds(okrtIds) {
  if (!Array.isArray(okrtIds) || okrtIds.length === 0) return [];
  const placeholders = okrtIds.map(() => '?').join(', ');
  return all(
    `SELECT okrt_id, jira_ticket_id, created_at FROM jira_link WHERE okrt_id IN (${placeholders})`,
    okrtIds
  );
}
