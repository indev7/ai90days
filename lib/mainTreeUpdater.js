...
/**
 * Refresh all groups data from the server
 * This should be called after group create/update/delete operations
 * @param {Function} setGroups - Zustand store setGroups function
 * @returns {Promise<void>}
 */
export async function refreshGroupsFromServer(setGroups) {
  try {
    const response = await fetch('/api/main-tree');
    if (!response.ok) {
      throw new Error('Failed to refresh groups data');
    }
    const data = await response.json();
    if (data.mainTree && data.mainTree.groups) {
      setGroups(data.mainTree.groups);
      console.log('Groups refreshed from server:', data.mainTree.groups.length);
    }
  } catch (error) {
