...
const phase18MigrationPath = resolve(__dirname, '..', 'Phase1', 'PGDB', 'migrate-to-phase18.sql');
const schemaSql = readFileSync(schemaPath, 'utf8');
const phase18Sql = readFileSync(phase18MigrationPath, 'utf8');
...
async function applyPhase18Migration() {
  console.log(`Ensuring Phase 18 migration (vision/mission columns) on target DB using ${phase18MigrationPath}...`);
  await targetPool.query(phase18Sql);
  console.log('Phase 18 migration applied to target DB.');
}
...
async function main() {
  ...
  try {
    ...
    await applySchema();
    await applyPhase18Migration();
    const user = await fetchUser();
    await upsertUser(user);

    console.log('\nDone. Schema replicated and user copied.');
  } catch (err) {
