...
function NodeTemplate(node, expandedGroupId, onNodeClick, groupDetails, currentUserId, onEditGroup) {
  ...
  const depthClass = depth >= 2 ? styles.depth2 : depth === 1 ? styles.depth1 : styles.depth0;
  ...
  return (
    <div ref={nodeRef} className={`${styles.nodeWrapper} ${depthClass}`}>
      <div 
        className={`${styles.nodeBox} ${isExpanded ? styles.expanded : ''}`}
        onClick={() => onNodeClick(d.id)}
        title={title}
      >
        <div className={styles.nodeTitle} title={title}>{title}</div>
...
export default function GroupsView({
  ...
  const handleZoomChange = (e) => {
    ...
    if (Number.isFinite(next)) {
      setZoom(Math.min(140, Math.max(80, next)));
    }
  };
        ...
        <div className={styles.chartShell}>
          <div className={styles.chartToolbar}>
            <div className={styles.toolbarGroup}>
              ...
              />
              <span className={styles.toolbarValue}>{zoom}%</span>
            </div>
            <div className={styles.toolbarGroup}>
              ...
              <div className={styles.densityButtons}>
                <button
                  type="button"
                  className={`${styles.densityButton} ${density === 'compact' ? styles.densityActive : ''}`}
                ...
                <button
                  type="button"
                  className={`${styles.densityButton} ${density === 'comfortable' ? styles.densityActive : ''}`}
